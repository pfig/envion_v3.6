...
def search_archive(keyword, rows=100):
    query = f'title:("{keyword}") AND mediatype:(audio)'
...
def load_counter(outdir):
    counter_path = os.path.join(outdir, COUNTER_FILENAME)
...
def load_history(outdir):
    history_path = os.path.join(outdir, HISTORY_FILENAME)
...
def main():
    ...
    parser.add_argument("--files-per-run", type=int, default=3)
    parser.add_argument("--slices-per-file", type=int, default=20)
    ...
    if args.fetch:
        ...
        for kw in args.keywords:
            print(f"Searching Archive.org for keyword: {kw}")
            try:
                ids = search_archive(kw, rows=100)
        ...
        print(f"Done. {files_found} file(s) processed. Microfiles saved in: {args.out}")
